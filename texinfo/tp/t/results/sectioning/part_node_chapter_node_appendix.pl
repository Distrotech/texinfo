...
$result_converted{'info'}->{'part_node_chapter_node_appendix'} = 'This is , produced by tp version from .
...
$result_converted{'xml'}->{'part_node_chapter_node_appendix'} = '<node name="Top"><nodename>Top</nodename><nodenext automatic="on">chapter node</nodenext><nodeup automatic="on">(dir)</nodeup></node>
