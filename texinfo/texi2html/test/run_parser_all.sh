...
# there are better ways
  ...
  for command_dir in $commands; do
    ...
    if [ "z$current" = 'ztexi' ]; then
      ...
      echo "perl -w -I $testdir/$srcdir_test/../../../tp -I $testdir/$srcdir_test/../../lib/Unicode-EastAsianWidth/lib/ -I $testdir/$srcdir_test/../../lib/libintl-perl/lib/ -I $testdir/$srcdir_test/../../lib/Text-Unidecode/lib/ $command_run $format_option --force --conf-dir $testdir/$srcdir_test/../../../tp/t/init/ --conf-dir $testdir/$srcdir_test/../../../tp/examples --conf-dir $testdir/$srcdir_test/../../../tp/init --conf-dir $testdir/$srcdir_test/../../maintained_extra --conf-dir $testdir/$srcdir_test/../../examples --conf-dir $testdir/$srcdir_test/../../formats --conf-dir $testdir/$srcdir_test/ --error-limit=1000 --set-init-var TEST=1 --output ${outdir}$dir/ -I $testdir/$srcdir_test/ -I $testdir/$srcdir_test/../ --set-init-var=DUMP_TEXI=1 --macro-expand=${outdir}$dir/$basename.texi $remaining_out_dir $src_file 2>${outdir}$dir/$basename.2" >> $logfile
      ...
      remaining_out_dir=`echo $remaining | sed 's,@OUT_DIR@,'"${outdir}$dir/"',g'`
      echo "$command $dir" >> $logfile
      echo "perl -w -I $testdir/$srcdir_test/../../../tp -I $testdir/$srcdir_test/../../lib/Unicode-EastAsianWidth/lib/ -I $testdir/$srcdir_test/../../lib/libintl-perl/lib/ -I $testdir/$srcdir_test/../../lib/Text-Unidecode/lib/ $command_run $format_option --force --conf-dir $testdir/$srcdir_test/../../../tp/t/init/ --conf-dir $testdir/$srcdir_test/../../../tp/examples --conf-dir $testdir/$srcdir_test/../../../tp/init --conf-dir $testdir/$srcdir_test/../../maintained_extra --conf-dir $testdir/$srcdir_test/../../examples --conf-dir $testdir/$srcdir_test/../../formats --conf-dir $testdir/$srcdir_test/ -I $testdir/$srcdir_test/ -I $testdir/$srcdir_test/../ --set-init-var L2H_FILE=$testdir/$srcdir_test/../../examples/l2h.init --error-limit=1000 --set-init-var TEST=1 $l2h_tmp_dir --output ${outdir}$dir/ $remaining_out_dir $src_file > ${outdir}$dir/$basename.1 2>${outdir}$dir/$basename.2" >> $logfile
